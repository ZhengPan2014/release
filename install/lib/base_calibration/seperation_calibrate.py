...
class CalibrateRobot:
    def __init__(self):
        ...
        if self.has_gyro:
            self.sub_imu  = rospy.Subscriber('imu_left', Imu, self.imu_cb)
        ...
        rospy.loginfo('inital_wall_angle %f'%self.inital_wall_angle)
        rospy.loginfo('imu_calibrate_time %f'%self.imu_calibrate_time)
...
def main():
    rospy.init_node('seperation_calibrate')
    ...
    for speed in (-0.3, -0.7, -1.0, -1.5):
        robot.align()
        odom = robot.calibrate(speed, imu_drift)

        odom_corr.append(odom)

    prev_odom = rospy.get_param("~prev_wheel_tracker", 0.42)
