...
def json2launch(json_data):
    for tag, value in list(json_data.items()):
        if isinstance(value, dict):
            json2launch(value)
            if value.has_key('node') or value.has_key('include'):
                value['@ns'] = "$(optenv " + tag + " " + tag + ")"
                if not json_data.has_key('group'):
                    json_data['group'] = []
                json_data['group'].append(value)
                del json_data[tag]
            else:
                value['@file'] = tag + ".launch"
                if not json_data.has_key('include'):
                    json_data['include'] = []
                json_data['include'].append(value)
                del json_data[tag]
        elif not isinstance(value, list):
            if not tag[:1] == "@":
                if not json_data.has_key('arg'):
                    json_data['arg'] = []
                json_data['arg'].append({"@name": tag, "@value": value})
                del json_data[tag]
    return json_data
...
def main():
    ...
    p.add_option(
        ...
        dest="launch", help="Format launch for xml2json")
