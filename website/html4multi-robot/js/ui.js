var UI = UI || {
	...
	display: (val) => {
		if (val.hasOwnProperty('property'))
		{
			...
			else
			{
				...
				else if (stageName === 'robotStage')
				{
					for (var s in stageObj)
					{
						console.log(s);
						topic.unsubscribe();
						DATA.stage.stage.removeChild(stageObj[s]);	
					}
				}
				else
				{
	...
	dispRobot: (message) => {
		...
		var robotId = message.robotId;
		if (!pos)
		{
			return;
		}
		var ori = TF.quaternionToTheta(message.orientation);
		if (!DATA.robotPoseStage[robotId])
		{
			DATA.robotPoseStage[robotId] = ICON.robot({});
			DATA.stage.stage.addChild(DATA.robotPoseStage[robotId]);
		}
		else
		{
			if (DATA.stage.stage.contains(DATA.robotPoseStage[robotId]));
			{
				DATA.stage.stage.addChild(DATA.robotPoseStage[robotId]);
			}
		}
		DATA.robotPoseStage[robotId].x = pos.x;
		DATA.robotPoseStage[robotId].y = pos.y;
		DATA.robotPoseStage[robotId].rotation = ori;
	},
