...
class imu_jy901
{
	constructor(params)
	{
		...
		this.params.pub_name = params_.pub_name || 'imu_data';
		
		this.params.port = params_.port || '/dev/ttyUSB0';
		
		this.params.frame_id = params_.frame_id || 'base_imu';
	...
	get launch()
	{
		if (!this.roslaunch)
		{
			this.roslaunch = 'roslaunch bringup imu_jy901.launch';
	...
	get prettyLaunchStr()
	{
		let prettyLaunch = '\x1B[7m' + 'roslaunch bringup imu_jy901.launch';
		for (let p in this.params)
		{
			prettyLaunch += '\n\t'
			prettyLaunch += p;
