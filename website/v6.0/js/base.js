var BaseEvent = BaseEvent || {
    ...
    //    NavEvent.Subscribe(NavEvent.TopicEnum.global_plannerTopic, BaseEvent.globalPlannerCallback);
    },
                ...
                switch (temp[0]) {
                    ...
                    case "user_status":
                        ...
                        if (temp[1] == 256) {
                            ...
                            $("#btn-plc").text("PLC Running Exception");
                        }
                        else if (temp[1] == 0 && temp[1] != "") {
                            $("#btn-plc").buttonMarkup({ icon: "hty-stop" });
                            $("#btn-plc").text("启动灭菌");
                        }
                        else if (temp[1] == 1) {
                            $("#btn-plc").buttonMarkup({ icon: "hty-error-stop" });
                            $("#btn-plc").text("PLC Stop Exception");
                        }
                        else {
...
$(function () {
    ...
    $('#hrg-navigation-main-page').live('pageshow', function (event, ui) {
        if (ui.prevPage[0].id == "hrg-login-page") {
            if (BaseEvent.FirstLoad) {
                ...
                var htmlcontents1 = "<a href='#hty-super-page' class='btn-hty' data-transition='slide' data-role='button' data-icon='carat-r' data-iconpos='right' style='background: #0ff;'>HTY-SUPER SD6</a>";
    ...
    NavEvent.Subscribe(NavEvent.TopicEnum.nav_ctrl_statusTopic, function (data) {
        ...
        switch (data.status) {
            case 1:
                if (data.waypoint_name.split('-')[0]=="map") {
