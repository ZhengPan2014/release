var NavEvent = NavEvent || {
    ...
    TopicEnum: {
        ...
        map_editTopic: { name: '/map_edit', messageType: "nav_msgs/OccupancyGrid", compression: "png" },
    ...
    */
     ...
     *@div    {string}  画布父容器ID
     */
    ShowMap: function (width, height, div, topic) {
        ...
        //polygon.addPoint({ x: 0, y: 0, z: 0 });

        //polygon.addPoint({ x: 107, y: 733.375, z: 0 });

        //polygon.addPoint({ x: 200, y: 733.375, z: 0 });

        //polygon.addPoint({ x: 200, y: 500, z: 0 });

        //var line = polygon.createLineShape({ x: 200, y: 375, z: 0 }, { x: 107, y: 733.375, z: 0 })

        //polygon.lineContainer.addChild(line);
    ...
    zoom: function(scene, ratio, centerX, centerY){
        var centerX = centerX;
        var centerY = centerY;
     ...
     *@data {Array}  地图各像素点值
     */
    SaveAsMapEdit: function (info, data) {
     ...
     *@trajectory {object} 站点信息 TrajectoryMessage
    */
    TrajectoryRemove: function (trajectory) {
    ...
    goFront: function () {
        ...
        this.Timer = setInterval(function () {
            velTopic.publish(NavEvent.TwistMessage(0.4, 0));
        }, 300);
    },
    /*
     *后退
    ...
    goLeft: function () {
        ...
        this.Timer = setInterval(function () {
            velTopic.publish(NavEvent.TwistMessage(0, 0.5));
        }, 300);
    },
    /*
     *右转
    ...
    Subscribe: function (topic, callback) {
        console.log("Subscribe:%s", topic.name);
    ...
    */
    ...
    Publish: function (topic, data) {
        ...
        for (var i = 1; i < arguments.length; i++) {
            ...
            topic.publish(msg);
            console.log("Publish:%s,%s", topic.name, arguments[i]);
        }
    },
    /*
     *发布Topic 任意消息
    ...
    TPublish: function (topic, data) {
        topic.publish(data);
        console.log("Publish:%s,%s", topic.name, data.goal_name);
    }
};
