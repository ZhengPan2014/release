var UI = UI || {
	...
	dispWaypoints: (message) => {
		...
		for (var i = 0; i < message.waypoints.length; i++)
		{
			...
			var waypointHtml = getWaypointDom(waypoint.name, waypointType);
	...
	updateMapList: (mapList) => {
		...
		if (DATA.loading && DATA.loading.key !== 'init')
		{
			DATA.loading = false;
		}
	},
	...
	rosModeHandle: (mode) => {
		...
		else if (mode === NAV.CmdEnum.SaveMap)
		{
			var mapnav = $('.map_nav');
			if (!isEqual(mapnav.css('height'), 35))
            {
	...
	brushStatusHandle: (tool) => {
		...
		switch(tool)
		{
			...
			case 'confirm':
				...
				{
					console.log('publishing obstacle')
				...
				setTimeout(function(){
					NAV.pubCmdString('save_map_edit', true);
				}, 500);
	...
	wpClickHandle: (event) => {
		...
		ALERT.confirm({
			title: `站点${waypointName}`,
			text: `确定要导航至站点${waypointName}?`,
			callback: function(){
...
{
	var type = type;
	if (!NAV.WaypointPrefix.hasOwnProperty(type))
	{
		type = undefined;
	}
    ...
    var patternForTraj = `<option class="wpsInTrajAdd" value="${fullName}">${fullName}</option>`;
    return {
...
{
	var waypoint = getWpInfoByName(fullName);
	if (!waypoint)
	{
	...
	var wpInfo = JSON.parse(waypoint.header.frame_id);
	var imgType = wpInfo.type;
	if (!NAV.WaypointPrefix.hasOwnProperty(imgType))
	{
		imgType = undefined;
	}
	var pattern = `<li style="height: 60px" id="${fullName}" class="selectedWp">
                    	<span class="sidebar_site_type_logo">
                            <img src="image/${imgType}.png" width="100%"/>
                        </span>
...
function changeScene(dom)
{
	...
	NAV.manipulateScene(NAV.CmdEnum.MapUpdate, name, true);
...
function delScene(dom)
{
	...
	NAV.manipulateScene(NAV.CmdEnum.MapDelete, name, true);
...
{
	clickFeedback($(dom).parent());
	var fullName = $(dom).next().text().trim();
	var waypoint = getWpInfoByName(fullName);
	var wpInfo = JSON.parse(waypoint.header.frame_id);
	if (waypoint)
	{
...
// get waypoint's info by name
// params: 
// 	1. string fullName: waypoint's name
// return:
//	yocs_msgs/Waypoint waypoint, false if not found 
function getWpInfoByName(fullName)
{
...
function updateWaypointDetailPage(waypoint)
{
	var wpInfo = JSON.parse(waypoint.header.frame_id);
	console.log(wpInfo);
	wpInfo.close_enough = parseFloat(wpInfo.close_enough);
	wpInfo.goal_timeout = parseFloat(wpInfo.goal_timeout);	
	$('#wp_detail_name').text(waypoint.name);
	$('#wp_detail_frame_id').text(wpInfo.type);
	$('#wp_detail_failure_mode').text(wpInfo.failure_mode);
	$('#wp_detail_close_enough').text(wpInfo.close_enough.toFixed(3));
	$('#wp_detail_goal_timeout').text(wpInfo.goal_timeout.toFixed(3));
